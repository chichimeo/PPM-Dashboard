  ...
  <div v-show="show">
    ...
    >
      ...
      >
        ...
        <tab :title="$l('components.formSchedule.threatNotify','Threat Notify')">
    ...
    >
      ...
      <div v-if="item.taskName === 'threat-notify'">
        <form-schedule-threat-notify :payload="item.payload" />
      </div>
...
import FormScheduleThreatNotify from './FormScheduleThreatNotify.vue'

export default {
  name: 'FormSchedule',
  components: { DatePicker, Tabs, Tab, FormScheduleMailReport, FormScheduleXmlReport, FormScheduleThreatNotify },
  props: {
    show: {
      type: Boolean,
      default: false
  ...
  watch: {
    selectedTitle (val, oldVal) {
      ...
      switch (val) {
        ...
        case 1:
          this.dummyTimestamp = (new Date()).getTime()
          this.item.taskName = this.options.threatNotify.name
          break
        case 2:
          this.dummyTimestamp = (new Date()).getTime()
    ...
    selectedTaskName (val, oldVal) {
      if (this.item.type === 'create') {
        switch (val) {
          case 'threat-notify':
            this.item.payload = this.options.threatNotify.payload
            break
          case 'mail-report':
